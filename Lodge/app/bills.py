...
@bill_blueprint.route('/bill/<int:page_no>')
@login_required
def get_bills(page_no=1):
	if page_no < 1:
		page_no = 1
	bills, total_pages = bill_table_data(page_no=page_no, closed=False)
	if bills == []:
		if page_no == 1:
			return render_template('bills.html', bills=bills, page_no=1, total_p=1)
		return redirect('/bill')
	return render_template('bills.html', bills=bills, page_no=page_no, total_p=total_pages)

@bill_blueprint.route('/bill/old')
@bill_blueprint.route('/bill/old/<int:page_no>')
@login_required
def get_old_bills(page_no=1):
	if page_no < 1:
		page_no = 1
	bills, total_pages = bill_table_data(page_no=page_no)
	if bills == []:
		if page_no == 1:
			return render_template('old_bills.html', bills=bills, page_no=1, total_p=1)
		return redirect('/bill/old')
	return render_template('old_bills.html', bills=bills, page_no=page_no, total_p=total_pages)
...
def all_bill_data(bill):
	...
	data = {
		...
		'contact_number':customer.contact_no,
		'in_time':bill.in_time.strftime('%Y/%m/%d %I:%M %p'),
		'out_time':bill.out_time.strftime('%Y/%m/%d %I:%M %p'),
		'advance':bill.advance,
		'cost':bill.charge,
